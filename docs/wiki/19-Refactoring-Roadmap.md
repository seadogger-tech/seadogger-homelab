...
### Reusable Pattern: non-root containers on RWO Ceph RBD PVCs

Validated 2026-08-03 while deploying a self-hosted MCP knowledge server.
That app was removed from the cluster on 2026-08-15, but the pattern it
surfaced applies to any future workload, so the lesson is kept here.
...
the mounted PVC(s) before the main container starts. Any future app image
...
The same deployment also validated the target GitOps pattern described in
Priority 1.5 / [#50](https://github.com/seadogger-tech/seadogger-homelab/issues/50):
Ansible creates only the ArgoCD `Application` object, and ArgoCD owns
every actual resource via Kustomize — no ingress fetched separately via
`get_url`, no manual `kubectl apply` outside the one-time Application
bootstrap.
